...
static int rtl8188eu_power_gpio = 0;
...
static int rtl8188eu_module_power(int onoff)
{
	...
	// the power supply is control by GPIO
	if (rtl8188eu_power_gpio != 0) {
		if (onoff)
			is_on = GPIOF_OUT_INIT_HIGH;
		else
			is_on = GPIOF_OUT_INIT_LOW;
		gpio_request_one(rtl8188eu_power_gpio, is_on, NULL);
		gpio_free(rtl8188eu_power_gpio);
		rtl8188eu_msg("rtl8188eu module power set by gpio and it is turned %s.\n", onoff ? "ON" : "OFF");
		return 0;
	}

	// the power supply is control by AXP
...
void rtl8188eu_gpio_init(void)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_PIO == type) {
		rtl8188eu_power_gpio = val.gpio.gpio;
		rtl8188eu_msg("module power use gpio %d control\n", rtl8188eu_power_gpio);
