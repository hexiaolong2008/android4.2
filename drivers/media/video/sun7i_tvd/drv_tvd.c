...
#endif
static int b_tvd_suspend;
int signal_status = 0;
int multi_channel_display_support = 0;
__u32 actual_size = 0;
...
{
	sunxi_mem_free(phy_addr, actual_size);
}
#endif
...
static int tvd_close(struct file *file)
{
	...
	multi_channel_display_support = 0;
	dev->opened=0;
	stop_generating(dev);
...
static __u8 get_signal_status()
{
	...
	dev = list_entry(devlist.next, struct tvd_dev, devlist);
	if (!dev) {
...
static int vidioc_g_fmt_type_private(struct file *file, void *priv,struct v4l2_format *format)
{
	...
	for(i=0;i<4;i++){
		if (dev->channel_index[i]) {
			//bit4 system, bit0 signal detected 
...
static int vidioc_s_fmt_type_private(struct file *file, void *priv,struct v4l2_format *format)
{
	...
	if ((dev->row > 1) || (dev->column > 1)) {
		multi_channel_display_support = 1;
	}
	switch (dev->format) {
	case TVD_PL_YUV422:
	case TVD_PL_YUV420:
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	for(j=0;j<4;j++){
		//新增多路即插即用支持，即2路或者两路以上的通道使能，
		//但是每次值显示一路，如：ch0 ch1都能使用，ch0有数据的
		//时候显示ch0画面，ch1有数据的时候显示ch1画面
		//跟两路按1*2或者2*1同时显示的时候是不同的
		if (!multi_channel_display_support) {
			if(dev->channel_index[j] == 1){
				dev->channel_irq = j;
				break;
			}
...
static int fetch_config(struct tvd_dev *dev)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[0] = 1;//default open channel0
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[1] = 1;//default open channel1
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[2] = 0;//default not open channel2
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[3] = 0;//default not open channel3
...
void tvd_late_resume(struct early_suspend *h)
{
	...
	//这个地方加的这个延时，主要是给品网调试AVIN休眠唤醒的时候，
	//为了避免唤醒后屏幕出现蓝屏或者白屏而加的一个延时。
	//需要验证一下现在是否还需要。
	msleep(1000);
	b_tvd_suspend = 0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	val = signal_status | (b_tvd_suspend << 4);
#else
	val = b_tvd_suspend;
#endif
